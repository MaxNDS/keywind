<#macro kw>
    ...
    <div 
        ...
        x-data="userCodeData"
        x-trap="trapFocus" 
        @inputchanged="update(substituteCharAt(userCode, $event.detail.index, $event.detail.value)); currentIndex = $event.detail.index;"
        @keyup="if (checkKey(event.key)) { trapFocus = currentIndex !== userCode.length - 1; $focus.next(); }"
        >
        <input 
